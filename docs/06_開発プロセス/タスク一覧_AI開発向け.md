...
## 1. 共通基盤開発
  ...
  - [ ] **1.1. 環境構築**
    - [ ] 1.1.1. Android (Kotlin + Jetpack Compose) プロジェクト初期設定
      - **参照ドキュメント:** `docs/01_概要・全体像/技術仕様書.md`
      - **テスト:**
        - [ ] プロジェクトが正常にビルドできること
        - [ ] 空の画面が表示されること
    - [ ] 1.1.2. バックエンド (FastAPI) プロジェクト初期設定
      - **参照ドキュメント:** `docs/01_概要・全体像/技術仕様書.md`
      - **テスト:**
        - [ ] FastAPIサーバーが起動し、ヘルスチェックAPIが正常にレスポンスを返すこと
    - [ ] 1.1.3. Firestore プロジェクトセットアップ
      - **参照ドキュメント:** `docs/01_概要・全体像/技術仕様書.md`, `docs/04_データ設計/データベース.md`
      - **テスト:**
        - [ ] バックエンドからFirestoreへの接続が確立できること

  - [ ] **1.2. クライアント共通処理**
    - [ ] 1.2.1. `device_id` 生成・永続化 (SharedPreferences)
      - **参照ドキュメント:** `docs/01_概要・全体像/技術仕様書.md`
      - **テスト (TC7):**
        - [ ] 初回起動時に `device_id` が生成され、SharedPreferencesに保存されること
        - [ ] 2回目以降の起動時には保存された `device_id` が使用されること
    - [ ] 1.2.2. Room データベース設定 (`apps` テーブルスキーマ定義、DAO作成)
      - **参照ドキュメント:** `docs/04_データ設計/データベース.md`
      - **テスト:**
        - [ ] `apps` テーブルのスキーマが正しく定義されていること
        - [ ] DAOのCRUD操作が正しく機能すること (単体テスト)
    - [ ] 1.2.3. API クライアントモジュール実装 (Retrofit/OkHttp等)
      - **参照ドキュメント:** `docs/03_API仕様/API詳細.md`
      - **テスト:**
        - [ ] 各APIエンドポイントへのリクエスト・レスポンスが正しく処理されること (モックサーバーを用いたテスト)
    - [ ] 1.2.4. グローバルナビゲーション設定 (Compose Navigation)
      - **参照ドキュメント:** `docs/02_画面設計/画面一覧.md`, `docs/02_画面設計/画面遷移図.md`
      - **テスト:**
        - [ ] 定義された画面間の遷移が正しく動作すること
    - [ ] 1.2.5. 汎用エラーハンドリング機構 (P06画面への連携含む)
      - **参照ドキュメント:** `docs/05_詳細仕様/エラーハンドリング.md`, `docs/02_画面設計/画面一覧.md` (P06)
      - **テスト:**
        - [ ] APIエラー発生時にP06画面へ遷移し、適切なエラーメッセージが表示されること
        - [ ] その他の予期せぬエラー発生時にP06画面へ遷移し、適切なエラーメッセージが表示されること

  - [ ] **1.3. バックエンド共通処理**
    - [ ] 1.3.1. Firestore クライアント初期化・接続設定
      - **参照ドキュメント:** `docs/04_データ設計/データベース.md`
      - **テスト:**
        - [ ] Firestoreへの接続が正常に行えること
    - [ ] 1.3.2. 共通リクエストバリデーション (`device_id`等)
      - **参照ドキュメント:** `docs/03_API仕様/API詳細.md`, `docs/05_詳細仕様/バリデーション.md`
      - **テスト:**
        - [ ] `device_id` が含まれないリクエストに対してエラーが返されること
    - [ ] 1.3.3. 共通エラーハンドリングミドルウェア
      - **参照ドキュメント:** `docs/05_詳細仕様/エラーハンドリング.md`
      - **テスト:**
        - [ ] 各種エラー発生時に定義されたエラーレスポンスが返されること
